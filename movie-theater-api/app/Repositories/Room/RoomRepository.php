...
class RoomRepository implements RoomRepositoryInterface
{
    ...
    public function getRoomByShowtime($movieId, $showTime, $showDate, $cinemaId)
    {
        ...
        return Room::whereHas('schedules.movies', function ($query) use ($movieId) {
            $query->where('movie_id', $movieId);
        })
            ->when($showTime, function ($query) use ($showTime) {
                $query->whereHas('schedules', function ($q) use ($showTime) {
                    $q->where('schedule_time', $showTime);
                });
            })
            ->when($showDate, function ($query) use ($showDate) {
                $query->whereHas('schedules', function ($q) use ($showDate) {
                    $q->where('schedule_date', $showDate);
                });
            })
            ->when($cinemaId, function ($query) use ($cinemaId) {
